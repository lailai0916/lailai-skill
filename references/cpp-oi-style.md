...
4. **简洁**：不写注释、调试代码、防御性编程；不加多余空格；只定义必要的常量和变量。
...
17. 小型变量定义在局部作用域，避免全局；**大型数组用全局**。通常用静态数组，按需 `vector`（如邻接表）。避免频繁临时变量。

## 命名

18. **snake_case**。小型变量简洁，通常 ≤ 3 字符；语义用**英文单词的辅音字母**，不用拼音。
19. 常量：数据规模 `N` / `M` / `K`（值 = 题目最大规模 $+5$）；模数 `mod`；无穷大 `inf=0x3f3f3f3f`（便于 `memset`）；无穷小 `eps`；`pi=acos(-1)`、`e=exp(1)`、`phi=(sqrt(5)+1)/2`。
20. 变量：多组数据 `T`，输出组号用 `_`；规模 `n`，操作次数 `m` / `q`，操作种类 `op`；循环变量 `i` / `j` / `k`。
21. 常用名：`ans`（答案）`cnt`（计数）`cur`（当前）`dep`（深度）`dis`（距离）`fa`（父）`len`（长）`mn` / `mx`（最值）`pos`（位置）`res`（返回）`rt`（根）`siz`（大小）`son`（子）`sum`（和）`tag`（标记）`tmp`（临时）`val`（价值）`vis`（访问）。
