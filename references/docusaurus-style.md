...
**题解美学（语料校准）**：追求**精简、优雅、"几句话说清"的高手感**，不长篇教学。能一两句讲透的思路绝不铺开；显然的题意 / 定义直接略过（"是个人都知道"）。判断标准是"像顶尖高手随手写的"，不是"面面俱到的讲义"。解题思路偏**连贯自然段**——但仅限**单线推导**；一旦出现**两种以上并列 / 互斥的转移或情况**（如区间 DP 的「拼接 / 折叠」、分类讨论的各 case），**必须用有序列表分点**（`1. 拼接：…；2. 折叠：…`，标签**纯文本 + 全角冒号、不加粗**，非末项用 `；`收尾），**别揉成「一是…二是…」一逗到底的长段——揉成连贯段是 AI-tell**。这与「少用加粗小标签罗列」不矛盾：要躲的是 `**X**：` 这种**加粗伪标题堆叠**，不是分点本身（分点要，加粗不要）。简单转移行内 `$...$` 带过即可，多步推导才上 `$$` 公式块。
