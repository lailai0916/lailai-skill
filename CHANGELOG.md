...
- `SKILL.md`：核心原则 4「结构清晰」深化为「结构清晰、多而不乱，但通俗易懂」（内容多时分层归组、不平铺一堆同级标题，但张弛有度、不什么都切成密集小标题，结构服务于易读）；核心原则 8「编辑而非重写」并入「整合而非追加」（补新材料先研读再融入合理位置、取精去糟，不末尾追加）；「项目定位」补一条「以身作则」（skill 自身文本须符合自定义规范）。
- `references/learning-style.md`：规则 3「结构化沉淀」深化为「结构化、分层次」并补反向的「别过度切碎、求通俗易懂」；新增规则 8「整合融合而非追加」并交叉链接 `maintenance-guide` 的「合并 > 新增」；推荐 / 不推荐 / Checklist 同步分层、防过度切碎、融合三点（Checklist 仍 ≤ 8）。
- `references/maintenance-guide.md`：「进化原则」补「以身作则」——改 skill 时顺手让被改文件符合规范、发现违规即修。

### Notes

- 来源：lailai 要求写任何文本都「内容充实、结构清晰、多而不乱」、却也「通俗易懂、不过度密集、不什么都加小标题」，整合已有内容时研读融合（取精去糟、合理归位）而非末尾追加，且 skill 须以身作则。
- 判重：「整合而非追加」在 skill 维护层面已由 `maintenance-guide` 的「反膨胀：合并 > 新增 / 判重优先」覆盖，故只补通用 / 笔记层面并交叉链接，不重复造条。本次以深化既有规则为主，净增行数趋零。

### Added

- `references/email-style.md`（新建）：中英文正式邮件格式——六段结构（主题 / 称呼 / 事由 / 正文 / 敬语 / 签名）、中文全角与英文半角标点、礼仪（CC/BCC、附件、回复、校对）、中英示例各一、Self-review Checklist 8 项。复用 `writing-style`（简洁 / 短句）、`ai-tone-blacklist`、`project-docs-style`（kebab-case），不重复。
- `SKILL.md`：reference 索引 + 适用场景 + frontmatter description 注册「邮件（中 / 英）」。来源：lailai 要求拟一份规范专业的中英文邮件格式。
...
- `references/writing-style.md`：「语气与风格」补一条——register 可按场景切换，但同一篇产出内部语气 / 用词 / 句式 / 详略须统一、不中途漂移；Self-review Checklist 第 1 项并入「全篇风格一致」（未增项，守 ≤ 8 上限）。来源：lailai 要求文本语言风格一致。

### Notes

- lailai 同时要求的「简洁」「语句简短」经判重已全覆盖（SKILL 核心原则、`writing-style.md`「每句 ≤ 40 字」等），未重复新增。
